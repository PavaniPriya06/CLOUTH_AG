...
import { FiSave, FiInfo, FiCreditCard, FiAlertCircle, FiCheckCircle } from 'react-icons/fi';
...
export default function AdminSettings() {
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            setLoading(true);
            try {
                const { data } = await api.get('/settings/upi');
    ...
    const handleSave = async (e) => {
        ...
        setSaving(true);
        try {
            await api.post('/settings', { key: 'upi', value: upiId });
            toast.success('UPI ID saved successfully! Payments will now go to this ID.');
        } catch (err) {
